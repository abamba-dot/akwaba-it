...
(function initContactFeedback() {
  ...
  const whatsappNumber = "2250705688744";
  ...
  const buildWhatsappUrl = () => {
    ...
    const lines = [
      "Bonjour AKWABA IT, je souhaite vous contacter pour un projet.",
      "",
      `Nom : ${fields.nom.value.trim()}`,
      `Email : ${fields.email.value.trim()}`,
      `Telephone : ${fields.telephone.value.trim() || "Non renseigne"}`,
      `Besoin : ${service}`,
      "",
  ...
  form.addEventListener("submit", (event) => {
    ...
    setStatus("WhatsApp va s'ouvrir avec votre message pre-rempli.", "success");
